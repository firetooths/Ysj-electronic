...
export const MaintenanceLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header title="سامانه سرویس و نگهداری" />
      <button
        onClick={toggleMobileMenu}
      ...
      <aside className={`fixed inset-y-0 right-0 w-64 bg-white p-4 shadow-xl z-40 transform transition-transform duration-300 ease-in-out md:hidden ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <nav className="space-y-2">
            <NavItem to="/maintenance" end={true} icon={<DashboardIcon className="ml-3 text-indigo-500 fa-lg" />} label="داشبورد نگهداری" />
            <NavItem to="/maintenance/list" icon={<ListIcon className="ml-3 text-indigo-500 fa-lg" />} label="برنامه زمان‌بندی" />
            <NavItem to="/maintenance/new" icon={<AddIcon className="ml-3 text-indigo-500 fa-lg" />} label="تعریف برنامه جدید" />
        </nav>
      </aside>
